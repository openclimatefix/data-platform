-- Temporal Tables trigger function (from https://github.com/nearform/temporal_tables)
-- All references to TSTZ or TIMESTAMPZ have been replaced with their non-TZ counterparts.
...
DECLARE
  ...
  commonColumns text[];
  time_stamp_to_use timestamp;
  range_lower timestamp;
  existing_range tsrange;
...
BEGIN
  ...
  END IF;
  IF holder.atttypid != to_regtype('tsrange') THEN
  ...
  IF TG_OP = 'UPDATE' OR TG_OP = 'INSERT' THEN
    manipulate := jsonb_set('{}'::jsonb, ('{' || sys_period || '}')::text[], to_jsonb(tsrange(time_stamp_to_use, null, '[)')));
